  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => { setOpen((v) => !v); if (!open) load(); }}
        className="relative grid h-10 w-10 place-items-center rounded-xl bg-panel text-ink/80 nm-raised-sm nm-press hover:text-ink"
        aria-label={`Notifications${count ? `, ${count} unread` : ""}`}>
        <Bell size={19} weight={count ? "fill" : "regular"} />
        {count > 0 && (
          <span className="absolute -right-1 -top-1 grid h-5 min-w-[20px] place-items-center rounded-full bg-accent px-1 text-[10px] font-bold text-onAccent">
            {count > 99 ? "99+" : count}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 z-30 mt-2 w-[360px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl bg-panel nm-raised">
          <div className="flex items-center justify-between border-b border-border/50 px-4 py-2.5">
            <p className="text-sm font-semibold text-ink">Notifications{count ? ` (${count})` : ""}</p>
          ...
          <div className="max-h-96 overflow-auto">
            {count === 0 ? (
              ...
              <button key={n.id} onClick={() => openRow(n)}
                className="flex w-full items-start gap-2.5 border-b border-border/40 px-4 py-2.5 text-left transition-colors hover:bg-muted/[0.06]">
          ...
          <button onClick={() => { setOpen(false); nav("/notifications"); }}
            className="w-full border-t border-border/50 px-4 py-2.5 text-center text-xs text-accent hover:bg-muted/[0.06]">
