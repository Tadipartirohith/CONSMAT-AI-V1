...
SCHEMA = (
    ...
    "  summary: one or two sentences on the procurement plan.\n"
    ...
    "hub-self supply worth considering. Only concrete, grounded suggestions from the given data.\n"
...
def analyze(context: dict) -> dict | None:
    ...
    if provider not in OPENAI_COMPAT and provider != "anthropic":
        return None
    if not key and provider != "openai-compat":
        return None
    ...
    try:
        if provider in OPENAI_COMPAT:
            ...
            j = _post_json(f"{base_url or 'https://api.anthropic.com/v1'}/messages",
                           ...
                           {"model": model, "max_tokens": 800, "system": SCHEMA,
