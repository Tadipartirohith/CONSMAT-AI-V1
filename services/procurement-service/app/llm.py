...
SCHEMA = (
    ...
    "vendor in the registry sells it, and point to the open-market options you list in alternatives so "
    "the hub can source it. Keep it brief.\n"
    ...
    "  alternatives: array of {material_id, suggestion}. Cover EVERY plan line, including ones marked "
    "'unavailable' (no registry vendor). For an unavailable item, do NOT stop at 'unavailable': look at "
    "market and external_offers for the SAME material and surface the best real product the hub can "
    "actually buy - name the seller/brand, the price, and where to get it - so procurement can proceed. "
    "Commodity materials (steel/TMT bars, cement, sand, aggregate, bricks) are interchangeable across "
    "brands, so other brands of the same material ARE valid substitutes (e.g. for 'JSW Neosteel TMT "
    "bars', suggest other TMT brands available in market/external_offers). For an available/procured "
    "line, suggest a cheaper genuine substitute of the SAME product type when the data shows one "
    "materially cheaper (say over 2%): state the seller/brand, price, and approx per-unit or line "
    "saving. NEVER cross a specialty boundary (e.g. never offer ordinary grey OPC/PPC as a substitute "
    "for white cement). external_offers prices are INDICATIVE (verify, not firm). Only if the open "
    "market was checked and truly nothing comparable exists for a line, say so for that line. Return an "
    "empty array only when there is genuinely nothing relevant to add for any line.\n"
